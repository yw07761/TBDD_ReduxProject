...
import { selectClother, selectPerson } from '../redux/clotherSlice';
...
const AssetExample = () => {
  ...
  const renderClother = ({ item }) => (
    <View style={styles.clotherItem}>
      <Image source={item.image} style={styles.clotherImage} />
      <Text>{item.name}</Text>
  ...
  return (
    <ScrollView>
      <View>
        ...
        {/* Hiển thị Person từ Redux */}
        <Text style={styles.sectionTitle}>Person</Text>
        <FlatList
          data={person} // Dữ liệu từ Redux
        ...
        {/* Hiển thị Clother từ Redux */}
        <Text style={styles.sectionTitle}>Clothes</Text>
        <FlatList
          data={clother} // Dữ liệu từ Redux
